...
<template>
  <section
    ...
    @mouseenter="stopRotation" @mouseleave="restartRotation">
    <div class="container flex min-h-14 items-center gap-4 py-1">
      ...
      <div class="relative min-w-0 flex-1 overflow-hidden">
        <Transition name="promo-swap" mode="out-in">
          <div v-if="activeItem" :key="`${activeIndex}-${activeItem.title}`"
            ...
            <a href="#" class="hidden text-xs font-semibold uppercase tracking-[0.22em] text-slate-800 transition hover:text-[color:var(--color-primary)] lg:inline-flex">
              {{ activeItem.cta }}
            </a>
          </div>
        </Transition>
      ...
      <div class="flex items-center gap-2">
        <button v-for="(item, index) in items" :key="item.title" type="button"
          :aria-label="`Voir la mise en avant ${index + 1}`" :class="[
