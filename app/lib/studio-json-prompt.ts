...
const BLOCK_GUIDE: { type: string; label: string; data: boolean; types: ContentType[]; fields: string }[] = [
  ...
  // Graphiques
  { type: 'bar', label: 'Barres', data: true, types: ['statsdata', 'article', 'survey'], fields: 'fieldMapping.xAxis (obligatoire, dimension), fieldMapping.yAxes (obligatoire, [colonnes numériques]), fieldMapping.series (regroupement), fieldMapping.aggregate (sum|avg|count|min|max) ou fieldMapping.aggregates ([{column,fn}]), fieldMapping.columnLabels, fieldMapping.valueLabels, config: stacked, showLegend, colors, orientation, barStyle (chart|progress), format, prefix, suffix' },
  { type: 'line', label: 'Lignes', data: true, types: ['statsdata', 'article', 'survey'], fields: 'fieldMapping.xAxis (obligatoire), fieldMapping.yAxes (obligatoire), fieldMapping.series, fieldMapping.aggregate/aggregates, fieldMapping.columnLabels, fieldMapping.valueLabels, config: smooth, showLegend, colors, lineFill, format' },
  { type: 'pie', label: 'Camembert', data: true, types: ['statsdata', 'article', 'survey'], fields: 'Mode "column" (défaut) : fieldMapping.label (colonne des parts) + fieldMapping.value (colonne numérique) + fieldMapping.aggregate. Mode "segments" (config.pieMode="segments") : fieldMapping.pieSegments ([{fn,column,label?}], fn parmi sum|avg|count|min|max|remainder). fieldMapping.valueLabels renomme les parts.' },
  // Données
  { type: 'table', label: 'Tableau', data: true, types: ['statsdata', 'article', 'survey'], fields: 'fieldMapping.columns (obligatoire, [colonnes dans l\'ordre]), fieldMapping.columnLabels ({colonne:libellé}), fieldMapping.valueLabels ({colonne:{valeurBrute:libellé}}), fieldMapping.columnFormats ({colonne:{format,align}}), config: sortable, showPagination, pageSize, rowLimit, sortColumn, sortDirection' },
  ...
  { type: 'record', label: 'Fiche', data: true, types: ['statsdata', 'article', 'survey'], fields: 'fieldMapping.columns (obligatoire), fieldMapping.recordTitleColumn (titre, défaut 1re colonne), fieldMapping.columnLabels, fieldMapping.valueLabels. Prend la 1re ligne après filtres + tri (config.sortColumn/sortDirection). config.title' },
  { type: 'related', label: 'Entités liées', data: true, types: ['statsdata', 'article', 'survey'], fields: 'fieldMapping.columns (1re = libellé de la puce, 2e optionnelle = valeur), fieldMapping.valueLabels. config: title, rowLimit, sortColumn, sortDirection' },
  { type: 'map', label: 'Carte', data: true, types: ['statsdata', 'article', 'survey'], fields: 'Coordonnées : soit fieldMapping.latColumn + fieldMapping.lngColumn, soit fieldMapping.mapPointColumn (colonne unique "lat, lon" / WKT / GeoJSON) + fieldMapping.mapPointOrder (latlng|lnglat). fieldMapping.mapTitleColumn (titre de la fiche de survol), fieldMapping.columns (colonnes libellé/valeur de la fiche), fieldMapping.columnLabels, fieldMapping.valueLabels, fieldMapping.mapColorColumn (couleur des points), fieldMapping.mapSizeColumn (taille des points, numérique). sources ([{id,datasetId}]), filters. config: title, mapBasemap (clair|plan|sombre|couleur|aerien|relief), mapMarkerColor (hex), mapAutoFit (bool), mapHeight (px)' },
  // Éditorial
