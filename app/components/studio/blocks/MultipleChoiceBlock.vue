    ...
    <template v-else>
      ...
      <div v-if="!readonly" class="flex flex-col gap-2">
        <label v-for="opt in options" :key="opt" class="flex items-center gap-2.5 text-sm text-[var(--studio-muted)]">
          <input type="radio" disabled class="h-4 w-4 border-[var(--studio-line-strong)]" />
          {{ opt }}
        </label>
      ...
      <div v-else-if="showForm()" class="flex flex-col gap-2.5">
        <label v-for="opt in options" :key="opt" class="flex cursor-pointer items-center gap-2.5 rounded-xl border border-[var(--studio-line)] px-3 py-2 text-sm text-[var(--studio-ink)] transition-colors hover:border-[var(--color-primary)]" :class="selected === opt ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/5' : ''">
          <input v-model="selected" type="radio" :value="opt" class="h-4 w-4 border-[var(--studio-line-strong)] text-[var(--color-primary)] focus:ring-[var(--color-primary)]/30" />
          {{ opt }}
        </label>
        <button
