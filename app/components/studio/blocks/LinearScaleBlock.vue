  ...
  <div class="flex flex-col gap-3.5">
    ...
    <div v-if="!readonly" class="flex flex-col gap-2.5">
      <div class="flex flex-wrap items-center gap-2">
        <span v-if="minLabel" class="text-[12px] text-slate-500">{{ minLabel }}</span>
        <span v-for="s in steps" :key="s" class="flex h-11 w-11 items-center justify-center rounded-xl border-[1.5px] border-slate-200 bg-[#faf9fd] text-[15px] font-bold text-slate-950">{{ s }}</span>
        <span v-if="maxLabel" class="text-[12px] text-slate-500">{{ maxLabel }}</span>
      </div>
    ...
    <div v-else-if="showForm()" class="flex flex-col gap-3.5">
      <div class="flex flex-wrap items-center gap-2">
        <span v-if="minLabel" class="text-[12px] text-slate-500">{{ minLabel }}</span>
        <button
          v-for="s in steps" :key="s"
          type="button"
          class="flex h-11 w-11 items-center justify-center rounded-xl border-[1.5px] text-[15px] font-bold transition-colors"
          :class="selected === s ? 'border-[var(--color-primary)] bg-[var(--color-primary)] text-white' : 'border-slate-200 bg-[#faf9fd] text-slate-950 hover:border-[var(--color-primary)]'"
          @click="selected = s"
        >
          {{ s }}
        </button>
      ...
      <button
        class="self-start rounded-full bg-[var(--color-primary)] px-6 py-2.5 text-sm font-bold text-white transition-opacity hover:opacity-90 disabled:opacity-40"
    ...
    <div v-else class="flex flex-col gap-2.5">
      <div class="flex items-baseline gap-2">
        <span class="text-2xl font-extrabold text-slate-950">{{ (form?.aggregate.value.average ?? 0).toFixed(1) }}</span>
        <span class="text-[12px] text-slate-400">/ {{ max }} · moyenne</span>
      </div>
      <div class="flex items-end gap-1.5">
        <div v-for="s in steps" :key="s" class="flex flex-1 flex-col items-center gap-1">
          <div class="flex h-16 w-full items-end overflow-hidden rounded-md bg-slate-100">
            <div class="w-full rounded-md bg-[var(--color-primary)]" :style="{ height: `${(countFor(s) / distributionMax) * 100}%` }" />
          </div>
          <span class="text-[11px] font-mono text-slate-400">{{ s }}</span>
        </div>
      </div>
