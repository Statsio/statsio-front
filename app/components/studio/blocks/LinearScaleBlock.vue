  ...
  <div class="flex flex-col gap-3">
    ...
    <div v-if="!readonly" class="flex flex-col gap-2">
      <div class="flex items-center gap-2">
        <span v-if="minLabel" class="text-xs text-[var(--studio-faint)]">{{ minLabel }}</span>
        <button v-for="s in steps" :key="s" disabled class="flex h-8 w-8 items-center justify-center rounded-full border border-[var(--studio-line)] text-xs text-[var(--studio-faint)]">{{ s }}</button>
        <span v-if="maxLabel" class="text-xs text-[var(--studio-faint)]">{{ maxLabel }}</span>
      </div>
    ...
    <div v-else-if="showForm()" class="flex flex-col gap-3">
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="selected === s ? 'border-[var(--color-primary)] bg-[var(--color-primary)] text-white' : 'border-[var(--studio-line)] text-[var(--studio-ink)] hover:border-[var(--color-primary)]'"
          @click="selected = s"
        >
          {{ s }}
        </button>
    ...
    <div v-else class="flex flex-col gap-2">
      <div class="flex items-center gap-3">
        <span class="text-2xl font-bold text-[var(--studio-ink)]">{{ (form?.aggregate.value.average ?? 0).toFixed(1) }}</span>
        <span class="text-xs text-[var(--studio-faint)]">/ {{ max }} · moyenne</span>
      </div>
      <div class="flex items-end gap-1.5">
        <div v-for="s in steps" :key="s" class="flex flex-1 flex-col items-center gap-1">
          <div class="flex h-16 w-full items-end overflow-hidden rounded bg-[var(--studio-note)]">
            <div class="w-full rounded bg-[var(--color-primary)]" :style="{ height: `${(countFor(s) / distributionMax) * 100}%` }" />
          </div>
          <span class="text-[10px] text-[var(--studio-faint)]">{{ s }}</span>
        </div>
      </div>
