...
<template>
  <div class="flex flex-col h-full">
    ...
    <!-- Search + refresh -->
    <div class="sticky top-0 z-10 bg-white px-[22px] pb-3">
      <div class="relative flex items-center gap-[9px] rounded-full border-[1.5px] border-[var(--studio-line-strong)] px-[15px] py-2.5 focus-within:border-[var(--color-primary)]">
        ...
        <input
          ...
          class="min-w-0 flex-1 bg-transparent pr-6 text-[13.5px] text-[var(--studio-ink)] placeholder:text-[var(--studio-faint)] focus:outline-none"
        />
        <button
          class="absolute right-3 top-1/2 -translate-y-1/2 rounded p-1 text-[var(--studio-faint)] transition-colors hover:text-[var(--studio-ink)]"
          title="Rafraîchir"
    ...
    <div class="flex flex-1 flex-col gap-2 overflow-y-auto px-[22px] py-2">
      ...
      <!-- Loading -->
      <div v-if="datasets.isLoading" class="flex items-center justify-center py-8 gap-2 text-sm text-[var(--studio-faint)]">
      ...
      >
        ...
        <p class="text-sm font-medium text-[var(--studio-muted)]">
      ...
      >
        ...
        <div v-if="deletingId === dataset.id" class="px-3 py-2.5 bg-red-50 flex flex-col gap-1.5">
          <div class="flex items-center gap-2">
            ...
            <button
              class="text-xs font-semibold text-[var(--studio-muted)] hover:text-[var(--studio-ink)] px-2 py-1 rounded transition-colors"
        ...
        <div
          v-else
          class="group flex items-center gap-2 px-3 py-2.5 cursor-pointer hover:bg-[var(--studio-note)] transition-colors"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold text-[var(--studio-ink)] truncate">{{ dataset.name }}</p>
            <div class="flex items-center gap-1.5 mt-0.5">
              <span class="w-1.5 h-1.5 rounded-full shrink-0" :class="statusConfig[dataset.status].dot" />
              <span class="text-[10px] text-[var(--studio-faint)]">
                ...
                <template v-if="dataset.status === 'ready' && dataset.materialization === 'live' && dataset.rowCount === 0"> · source en direct</template>
                <template v-else-if="dataset.status === 'ready'"> · {{ formatRows(dataset.rowCount) }} lignes</template>
              </span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
            <button
              ...
              class="p-1 rounded text-[var(--studio-faint)] hover:text-[var(--studio-muted)] hover:bg-[var(--studio-wash)] transition-colors"
            ...
            <button
              ...
              class="p-1 rounded text-[var(--studio-faint)] hover:text-[var(--studio-muted)] hover:bg-[var(--studio-wash)] transition-colors disabled:opacity-50"
            ...
            <button
              class="p-1 rounded text-[var(--studio-faint)] hover:text-red-500 hover:bg-red-50 transition-colors"
          ...
          <!-- Chevron -->
          <svg
            class="w-4 h-4 text-[var(--studio-faint)] shrink-0 transition-transform"
        ...
        <div v-if="expandedId === dataset.id && deletingId !== dataset.id" class="border-t border-[var(--studio-line)] bg-[var(--studio-note)] px-3 py-2">
          <div v-if="datasets.isLoadingSchema(dataset.id)" class="text-xs text-[var(--studio-faint)] py-2 text-center">
            Chargement du schéma…
          </div>
          <template v-else-if="getSchema(dataset.id)">
            <p class="text-[10px] font-bold uppercase tracking-wider text-[var(--studio-faint)] mb-1.5">Colonnes</p>
            <div class="flex flex-col gap-1">
              ...
              >
                <span class="font-mono text-[11px] text-[var(--studio-muted)] truncate flex-1">{{ col.name }}</span>
                <span
              ...
              <p v-if="(getSchema(dataset.id)?.columns.length ?? 0) > 8" class="text-[10px] text-[var(--studio-faint)] mt-1">
            ...
            <div v-if="datasets.getPreview(dataset.id)" class="mt-3">
              <p class="text-[10px] font-bold uppercase tracking-wider text-[var(--studio-faint)] mb-1.5">
                Aperçu
                <span class="font-normal normal-case">({{ datasets.getPreview(dataset.id)!.rows.length }} lignes)</span>
              </p>
              <div class="overflow-x-auto overflow-y-auto max-h-40 rounded-lg border border-[var(--studio-line)] bg-white">
                  ...
                  <thead>
                    <tr>
                      <th
                        ...
                        :key="col"
                        class="px-2 py-1.5 text-left font-semibold text-[var(--studio-muted)] border-b border-[var(--studio-line-strong)] whitespace-nowrap"
                      >{{ col }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    >
                      <td
                        ...
                        :key="ci"
                        class="px-2 py-1.5 text-[var(--studio-ink)] font-mono truncate max-w-[120px]"
              ...
              <p v-if="datasets.getPreview(dataset.id)!.total > datasets.getPreview(dataset.id)!.rows.length" class="text-[10px] text-[var(--studio-faint)] mt-1">
                +{{ datasets.getPreview(dataset.id)!.total - datasets.getPreview(dataset.id)!.rows.length }} lignes supplémentaires
              </p>
            </div>
            <div v-else-if="datasets.isLoadingPreview(dataset.id)" class="text-xs text-[var(--studio-faint)] py-2 text-center">
              Chargement de l'aperçu…
            </div>
            <div v-else-if="datasets.getPreviewError(dataset.id)" class="mt-3">
              <p class="text-[10px] font-bold uppercase tracking-wider text-[var(--studio-faint)] mb-1.5">Aperçu</p>
              <p class="text-[11px] text-[var(--studio-faint)] italic">Aperçu indisponible — {{ datasets.getPreviewError(dataset.id) }}</p>
            </div>
          </template>
        </div>
      </div>
    ...
    <div class="shrink-0 bg-white px-[22px] py-3.5">
      <button class="studio-add-btn !py-[15px] !text-[13px]" @click="showModal = true">
        + Ajouter une source
      </button>
    </div>
  </div>
