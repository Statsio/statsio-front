...
<template>
  <PageHero ref="heroRef" dot-color-rgb="139,92,246">
    <template #content>
      <AppBadge data-anim="badge" variant="soft-primary" mono>{{ content.heroBadge }}</AppBadge>
      ...
      <div data-anim="ctas" class="flex flex-wrap items-center gap-3">
        <AppButton as="router-link" :to="primaryCtaTo" variant="primary" size="lg">
          {{ content.heroCtaPrimary }}
        </AppButton>
        <AppButton as="router-link" :to="secondaryCtaTo" variant="secondary" size="lg">
    ...
    <template #card>
      <div class="mb-4 flex items-center justify-between">
        <span class="text-[13px] font-bold text-slate-900">{{ content.heroChartLabel }}</span>
        <span class="mono text-[11px] font-semibold text-emerald-600">▲ 3.2%</span>
      </div>
      <AppSparkline :points="content.heroChartPoints" :height="110" />
      <div class="mono mt-1.5 flex justify-between text-[10.5px] text-slate-400">
        <span v-for="marker in content.heroChartMarkers" :key="marker">{{ marker }}</span>
      </div>
    </template>
  </PageHero>
</template>
