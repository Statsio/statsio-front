...
const sections: NavSection[] = [
  ...
  {
    title: 'Data',
    items: [{ label: 'Provenances', to: '/admin/data/provenances' }],
  },
  {
    title: 'Support',
    items: [{ label: 'Messages de contact', to: '/admin/contact-messages' }],
  },
  {
    title: 'MediStats',
    soon: true,
...
<template>
  <aside class="flex h-full w-60 flex-col bg-slate-950 text-slate-300">
    ...
    <div class="flex h-16 shrink-0 items-center gap-2.5 border-b border-slate-800 px-5">
      <div
        class="flex h-7 w-7 items-center justify-center rounded-lg bg-white/10 text-xs font-bold text-white"
      >
        S
      </div>
      <div>
    ...
    <nav class="flex-1 overflow-y-auto py-4">
      <div v-for="section in sections" :key="section.title" class="mb-6">
        <p class="mb-1.5 px-5 text-[10px] font-bold uppercase tracking-[0.15em] text-slate-600">
          {{ section.title }}
        </p>
        ...
        >
          <span
            ...
            :class="isActive(item.to) ? 'bg-white' : 'bg-slate-700'"
          />
          {{ item.label }}
        </RouterLink>
      </div>
    ...
    <div class="shrink-0 border-t border-slate-800 p-4">
      <RouterLink
        to="/"
        class="flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-medium text-slate-500 transition hover:bg-white/5 hover:text-slate-300"
      >
