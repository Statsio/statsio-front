...
const allCategories = ref<ChannelCategoryItem[]>([])
...
onMounted(async () => {
  try {
    allCategories.value = await getChannelCategories()
  } finally {
    loading.value = false
  }
})

// Quand le domaine change, retire de la sélection les catégories hors domaine.
watch(
  () => props.domain,
  (domain) => {
