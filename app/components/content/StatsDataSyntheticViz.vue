...
/**
 * Visuel décoratif d'une carte Statsdata (KPI factice + un des 5 modes de mini-graphe),
 * déterministe sur `item.id`. Aucune donnée réelle — à ne monter qu'en `mode="public"`.
 */
...
const axisFrom = computed(() =>
  props.item.category === 'climat' ? '2019' : props.item.category === 'economie' ? 'J-14' : '2024',
)
const axisTo = computed(() =>
  props.item.category === 'climat' ? '2026' : props.item.category === 'economie' ? 'auj.' : '2026',
)
...
const barHeights = computed(() =>
  Array.from({ length: 12 }, (_, i) => {
    ...
    const h = 20 + r * 75
    return { h: h.toFixed(1) + '%', color: i >= 9 ? palette.value[0] : '#dcd6ee' }
  }),
)
...
const rowLabels = computed(() => {
  const dflt = [
    ...
    ['Finance', 'Industrie', 'Commerce', 'Services'],
    ['PACA', 'IdF', 'AURA', 'HDF'],
  ]
  return dflt[seed.value % dflt.length] ?? dflt[0]!
})
  ...
  <div class="my-4 rounded-[14px] bg-[#faf9fd] p-3.5">
    ...
    <div v-else-if="chartMode === 'bars'">
      <div class="h-[74px] flex items-end gap-1">
        <span v-for="(b, i) in barHeights" :key="i" class="min-w-0 flex-1 rounded-t-[4px]" :style="{ height: b.h, background: b.color }" />
      </div>
      <div class="mt-[7px] flex justify-between font-mono text-[9.5px] text-slate-400">
        <span>{{ axisFrom }}</span><span>{{ axisTo }}</span>
      </div>
